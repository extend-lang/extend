 ...
 */
template<>
enum struct ParserTraits<ParserType::TEXTCODE>::TOK {
...
template<>
constexpr bool ParserTraits<ParserType::TEXTCODE>::isIndentSupported = false;

#define DECLARE_TOKEN(NAME, TYPE)                                              \
  template<>                                                                   \
  template<>                                                                   \
  struct ParserTraits<ParserType::TEXTCODE>::UnderlyingType<                   \
    ParserTraits<ParserType::TEXTCODE>::TOK::NAME>                             \
  {                                                                            \
    using Type = TYPE;                                                         \
  };
#include "token_types.h"
...
template<>
struct ParserTraits<ParserType::TEXTCODE>::ParsedValue
{
  using FromLexer =
    variant<
...
using TOK = text::ParserTraits<text::ParserType::TEXTCODE>::TOK;
}
